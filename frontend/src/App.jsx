        ...
        <div style={{ maxWidth: '680px', width: '92%', padding: '32px', background: 'var(--bg-card)', border: '1px solid var(--border-glow)', borderRadius: 'var(--radius-lg)', boxShadow: '0 12px 40px rgba(0,0,0,0.6)' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '0.92rem', fontWeight: 700 }}>Run Worker Cell in Kaggle Notebook</span>
            </div>
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '10px' }}>
              Copy this Python code into your Kaggle Notebook (with GPU accelerator enabled) and run it:
            </p>

            <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
              <button 
                className={`script-tab-btn ${activeTunnelTab === 'cloudflare' ? 'active' : ''}`}
                onClick={() => setActiveTunnelTab('cloudflare')}
              >
                Cloudflare Tunnel (Recommended)
              </button>
              <button 
                className={`script-tab-btn ${activeTunnelTab === 'pinggy' ? 'active' : ''}`}
                onClick={() => setActiveTunnelTab('pinggy')}
              >
                Pinggy SSH Tunnel
              </button>
            </div>
            ...
            <div className="code-box-container">
              <button className="copy-badge-btn" onClick={() => copyScriptCode(currentScript)}>
                {copiedCode ? <Check size={14} style={{ color: '#10b981' }} /> : <Copy size={14} />}
                {copiedCode ? 'Copied!' : 'Copy Cell Code'}
              </button>
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '0.92rem', fontWeight: 700 }}>Paste Remote Tunnel URL</span>
            </div>
